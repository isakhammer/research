...
\end{tcolorbox}
 ...
 \]
  ...
  \end{theorem}
   ...
   \newpage
   \section{Lecture 11/09 }%
   \label{sec:varmeligning}

   \begin{definition}
     \textbf{Varmeligning} . \[
     \begin{split}
       u_{t} - \Delta  u &=  0 , \quad  \textbf{x}  \in  \mathbb{R} ^{n}  \\
       u_{t} - u_{xx} &=  0 \\
     \end{split} 
     \] 
   \end{definition}
   
   Anta at $u$ løser $u_{t} - u_{xx} =0$ \[
   u\left( t,x \right) = u\left( \lambda ^2 t , \lambda  x \right)
   \] 
   blir en løsning $\lambda  > 0$ . \[
     \begin{split}
   \int_{\mathbb{R} }^{}  v\left( t,x \right) dx   &=  \int_{\mathbb{R} }^{}  \left( \lambda  ^2 t, \lambda  x \right)  \\
   &=  \frac{1}{\lambda }  \int_{\mathbb{R} }^{}  u\left( \lambda  ^2 t, y \right)  \\
     \end{split} 
   \] 
   Gjør istedet \[
   \begin{split}
     v\left( t,x \right) &=  \frac{1}{\lambda } u\left( \lambda ^2t , \lambda x \right) \\
     \text{Så blir} \quad  \int_{\mathbb{R} }^{}  v\left( t,x \right) dx &=  \int_{\mathbb{R} }^{ }  u\left( \lambda  ^2 t , x  \right) dx  \\   \\
   \end{split} 
   \] 
   Løsning $u$ kalles  \textbf{selv simular}  dersom \[
   \begin{split}
     u\left( t,x \right) &=  \frac{1}{\lambda } u\left( \lambda ^2 t , \lambda  x \right) \quad  \text{ for alle } \quad  t > 0 , x \in  \mathbb{R}  , \lambda  > 0   \\
   i\end{split} 
   \] 

   Hvis u er selsimulæør er \[
   u\left( t,x \right) = \frac{1}{ \sqrt{t} } u\left( 1, \frac{x}{\sqrt{t} } \right) , \quad  \lambda  = \frac{1}{\sqrt{t} } 
   \] 
   ...
   \begin{tcolorbox}
     \textbf{Konvolusjon.}  \[
     f \circ g\left( \mathbf{x} \right) =  \int_{\mathbb{R} ^{n}}^{}  f\left( \mathbf{x} - \xi  \right) g\left( \mathbf{\xi } \right) d^{n} \mathbf{\xi }  \\
     \] 
     Er ok dersom $\quad f,g \in  L' \left( \mathbb{R} ^{n} \right) $ dvs \[
     \int_{\mathbb{R} ^{n}}^{}  \|f\|_{}^{} d^{n} \mathbf{x} < \infty
     \] 
     $f\circ g \in  L' $ \[
     \begin{split}
       f \circ g &= g \circ  f \\
     ...
     \end{split} 
     \] 
     Løsning av Cauchy Problemer \[
     u\left( t, \mathbf{x} \right) = H_{t} \circ  g\left( \mathbf{x} \right)
     \] 
   \end{tcolorbox}

   \[
   H_{\tau _{2} + \tau _{1}} \circ g = H_{\tau _{2}} \circ H_{\tau _{1}} \circ g
   \] 
  \begin{theorem}
    \[
    H_{\tau _{2} + \tau _{1}} = H_{\tau _{2}} \circ H_{\tau _{1}}
    \] 
  \end{theorem}

  Semigruppe egenskaper \[
  e^{a + b} = e^{ a} + e^{b}
  \] 
  La $A$ være $n \times  n $ matrise \[
    \begin{split}
  e^{A}  & = \sum_{n= 0}^{ \infty}  \frac{A^{n}}{ n!}  \\
  e^{A + B} = e^{A} e^{B} , \quad  \text{er ikke alltid sant}  \\
  e^{\left( s+ t \right) A} &=  e^{sA} e^{tA} \\
    \end{split} 
  \] 
  ...
  \textbf{Entydighet?}  \[
    \begin{rcases}
      ...
      u\left( 0, \mathbf{x} \right) &=  g\left( \mathbf{x} \right) 
    \end{rcases}
    \text{ Har ikke entydig løsning!!}
  \] 
  Men dersom $g$ er bregrenset så finnes kun en begrenset løsning ! 
  
  \newpara
  Hvis vi lar \[
    \begin{split}
  u_{t} - u _{xx} &=  0 \\
  u\left( 0, \mathbf{x} \right)  &  = 0
    \end{split} 
  \] 
  En typisk ikketriviell løsning: \[
  u\left( t,x \right) e^{-Ax^2 } 
  \] 
  blir ubegrenset.
